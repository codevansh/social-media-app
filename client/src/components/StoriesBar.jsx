        ...
        <div className='w-screen no-scrollbar px-4 py-4 lg:max-w-2xl overflow-x-auto sm:[calc(100vw-240px)]'>
            <div className='flex gap-4 pb-5 '>
                ...
                {stories.map((story, index) => (
                    <div onClick={() => setViewStory(story)} key={index} className='relative rounded-lg shadow w-40 h-40 overflow cursor-pointer hover:shadow-lg transition-all duration-200 bg-linear-to-r from-indigo-500 to-purple-600 hover:from-indigo-700 hover:to-purple-800 active:scale-95'>
                            ...
                            story.media_type !== 'text' && (
                                <div className='absolute inset-0 z-1 rounded-lg bg-black overflow-hidden'>
                                    {story.media_type === "image" ?
                                        <img src={story.media_urls?.[0]} alt="" className='h-full w-full object-cover hover:scale-110 transition duration-500 opacity-100 hover:opacity:80' />
                                        :
                                        <video src={story.media_urls?.[0]} className='h-full w-full object-cover hover:scale-110 transition duration-500 opacity-70 hover:opacity:80' />
                                    }
                                </div>
                            )
                        }
